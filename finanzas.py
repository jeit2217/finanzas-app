...
st.markdown("<style>.tarjeta-saldo { background: linear-gradient(135deg, #1f4068 0%, #162447 100%); color: white; padding: 20px; border-radius: 15px; text-align: center; margin-bottom: 15px; border: 1px solid rgba(255,255,255,0.1);}</style>", unsafe_allow_html=True)
...
# --- APP PRINCIPAL ---
    ...
    # HEADER
    st.markdown(f'<div class="tarjeta-saldo"><h3>DISPONIBLE</h3><h1>${total:,}</h1></div>', unsafe_allow_html=True)
    if st.button("🚪 Salir"):
    ...
    with p_mov:
        ...
        if modo == "ing":
            ...
            if st.button("Guardar Ingreso"):
                ...
                hist.append({"tipo": "Ingreso", "banco": b_i, "monto": m_i, "cat": "Ingreso", "det": d_i})
                guardar_datos("hist", hist)
                st.rerun()
        else:
            st.markdown("### Registrar Gasto")
            b_g = st.selectbox("De donde sale:", BANCOS)
            cat_g = st.selectbox("Categoría:", CATEGORIAS)
            m_g = st.number_input("Monto:", min_value=0, step=1000)
            paga_d = st.checkbox("¿Paga una deuda?")
            id_d = st.text_input("ID Deuda:") if paga_d else ""
            ...
            if st.button("Confirmar Gasto"):
                if m_g <= saldos[b_g]:
                    ...
                    hist.append({"tipo": "Gasto", "banco": b_g, "monto": m_g, "cat": cat_g, "det": "Abono Deuda" if paga_d else "Gasto"})
                    guardar_datos("hist", hist)
                    st.rerun()
                else: st.error("No tienes saldo.")

    # --- PESTAÑA DEUDAS ---
    with p_deu:
        st.subheader("Mis Deudas")
        id_n = st.text_input("ID Deuda (ej: JUAN):").upper()
        m_n = st.number_input("Monto Deuda:", min_value=0)
        if st.button("Crear Deuda"):
            deudas[id_n] = {"nombre": id_n, "monto": m_n}
            guardar_datos("deudas", deudas)
            st.rerun()
        
        for k, v in deudas.items():
            st.markdown(f'<div class="item-historial" style="border-left-color:#e63946"><b>ID: {k}</b> - Debe: ${v["monto"]:,}</div>', unsafe_allow_html=True)
